...
CASES = [
    ...
    (
        ...
        IssueType.ORDER_SUBMISSION_FAILURE,
        "ORDER_SCREEN_STUCK",
        "매도 확인 후 다음 화면으로 넘어가지 않음",
    ),
    (
        "submission_03",
        IssueType.ORDER_SUBMISSION_FAILURE,
        "ORDER_SCREEN_STUCK",
        "주문 접수 단계에서 계속 대기 상태가 표시됨",
    ),
    (
        "submission_04",
        IssueType.ORDER_SUBMISSION_FAILURE,
        "ORDER_SCREEN_STUCK",
        "확인 버튼을 눌러도 주문 화면이 반응하지 않음",
    ),
    (
        "submission_05",
        IssueType.ORDER_SUBMISSION_FAILURE,
    ...
    (
        ...
        "ORDER_RESULT_MISSING",
        "매도 주문 후 체결 여부를 확인할 수 없음",
    ),
    (
        "result_02",
        IssueType.ORDER_RESULT_UNCONFIRMED,
        "ORDER_RESULT_MISSING",
        "주문을 넣었지만 접수되었는지 알 수 없음",
    ),
    (
        "result_03",
        IssueType.ORDER_RESULT_UNCONFIRMED,
        "ORDER_RESULT_MISSING",
        "주문 완료 화면이 나오지 않아 접수 여부를 알 수 없음",
    ),
    (
        "result_04",
        IssueType.ORDER_RESULT_UNCONFIRMED,
        "ORDER_RESULT_MISSING",
        "주문번호가 나타나지 않아 주문 결과가 불확실함",
    ),
    (
        "result_05",
        IssueType.ORDER_RESULT_UNCONFIRMED,
        "ORDER_RESULT_MISSING",
        "주문을 시도했으나 처리 결과를 확인하지 못함",
    ),
    (
        "login_01",
        IssueType.LOGIN_ACCESS_FAILURE,
        "LOGIN_CREDENTIAL_FAILURE",
        "비밀번호를 입력해도 로그인이 실패함",
    ),
    (
        "login_02",
        IssueType.LOGIN_ACCESS_FAILURE,
        "APP_LOGIN_FAILURE",
        "M-able 앱에 로그인이 되지 않음",
    ),
    (
        "login_03",
        IssueType.LOGIN_ACCESS_FAILURE,
        "AUTH_CODE_MISSING",
        "인증번호가 오지 않아 로그인할 수 없음",
    ),
    (
        "login_04",
        IssueType.LOGIN_ACCESS_FAILURE,
    ...
    (
        ...
        IssueType.LOGIN_ACCESS_FAILURE,
        "LOGIN_CREDENTIAL_FAILURE",
        "비밀번호가 올바른데도 계정 로그인이 거절됨",
    ),
    (
        "login_06",
        IssueType.LOGIN_ACCESS_FAILURE,
        "APP_LOGIN_FAILURE",
        "앱 로그인 화면에서 접속이 계속 실패함",
    ),
    (
        "login_07",
        IssueType.LOGIN_ACCESS_FAILURE,
        "APP_LOGIN_FAILURE",
        "M-able 로그인 버튼을 눌러도 접속되지 않음",
    ),
    (
        "login_08",
        IssueType.LOGIN_ACCESS_FAILURE,
        "AUTH_CODE_MISSING",
        "로그인 인증 문자가 휴대전화로 오지 않음",
    ),
    (
        "login_09",
        IssueType.LOGIN_ACCESS_FAILURE,
        "AUTH_CODE_MISSING",
        "본인인증 번호를 받지 못해 로그인 단계에서 막힘",
    ),
    (
        "balance_01",
    ...
    (
        ...
        IssueType.BALANCE_INQUIRY_ERROR,
        "BALANCE_NOT_REFRESHED",
        "보유 주식 수량이 잔고에 표시되지 않음",
    ),
    (
        "balance_03",
        IssueType.BALANCE_INQUIRY_ERROR,
        "EXECUTION_HISTORY_EMPTY",
        "체결 내역을 조회하면 빈 화면이 나타남",
    ),
    (
        "balance_04",
        IssueType.BALANCE_INQUIRY_ERROR,
        "ORDER_HISTORY_ERROR",
        "주문 내역 조회 화면에서 오류가 발생함",
    ),
    (
        "balance_05",
        IssueType.BALANCE_INQUIRY_ERROR,
        "BALANCE_NOT_REFRESHED",
        "보유 종목과 수량이 잔고 화면에 갱신되지 않음",
    ),
    (
        "balance_06",
        IssueType.BALANCE_INQUIRY_ERROR,
        "EXECUTION_HISTORY_EMPTY",
        "체결 목록을 열어도 거래 기록이 보이지 않음",
    ),
    (
        "balance_07",
        IssueType.BALANCE_INQUIRY_ERROR,
        "EXECUTION_HISTORY_EMPTY",
        "체결 내역 화면에 아무 항목도 표시되지 않음",
    ),
    (
        "balance_08",
        IssueType.BALANCE_INQUIRY_ERROR,
        "ORDER_HISTORY_ERROR",
        "주문내역 메뉴를 조회하면 오류 화면이 나타남",
    ),
    (
        "balance_09",
        IssueType.BALANCE_INQUIRY_ERROR,
        "ORDER_HISTORY_ERROR",
        "과거 주문 기록을 불러오는 중 조회 오류가 발생함",
    ),
    (
        "network_01",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "WIFI_CONNECTION_FAILURE",
        "와이파이 연결이 끊길 때 주문 화면이 멈춤",
    ),
    (
        "network_02",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "MOBILE_DATA_FAILURE",
        "모바일 데이터가 불안정하면 앱 접속이 실패함",
    ),
    (
        "network_03",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "DEVICE_SPECIFIC_FAILURE",
        "내 휴대전화에서만 M-able 화면이 열리지 않음",
    ),
    (
        "network_04",
        IssueType.DEVICE_NETWORK_SUSPECTED,
    ...
    (
        ...
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "WIFI_CONNECTION_FAILURE",
        "와이파이가 끊어진 뒤 주문 화면이 응답하지 않음",
    ),
    (
        "network_06",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "WIFI_CONNECTION_FAILURE",
        "무선 인터넷 연결이 끊기면 앱 화면이 멈춤",
    ),
    (
        "network_07",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "MOBILE_DATA_FAILURE",
        "LTE 연결이 불안정할 때 앱에 접속할 수 없음",
    ),
    (
        "network_08",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "MOBILE_DATA_FAILURE",
        "모바일 네트워크 상태가 나쁘면 로그인에 실패함",
    ),
    (
        "network_09",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "DEVICE_SPECIFIC_FAILURE",
        "다른 기기에서는 되지만 내 스마트폰에서만 앱이 실행되지 않음",
    ),
    (
        "network_10",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "DEVICE_SPECIFIC_FAILURE",
        "특정 휴대전화에서만 M-able 접속 화면이 열리지 않음",
    ),
    (
        "network_11",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "NETWORK_ERROR_MESSAGE",
        "앱에 네트워크 접속 오류 안내가 나타남",
    ),
    (
        "network_12",
        IssueType.DEVICE_NETWORK_SUSPECTED,
        "NETWORK_ERROR_MESSAGE",
        "통신 연결 실패 메시지가 표시되어 진행할 수 없음",
    ),
...
def describe(scores):
    if not scores:
        return "count=0"
    return (
        f"count={len(scores)} min={min(scores):.6f} mean={mean(scores):.6f} max={max(scores):.6f}"
    )
...
def main():
    ...
    cluster_counts = Counter(
        cluster_label for _, _, cluster_label, _ in CASES if cluster_label is not None
    )
    ...
    for index, (case_id, issue_type, cluster_label, text) in enumerate(CASES, 1):
        ...
        print(f"[{index:02d}/{len(CASES)}] embedding: {case_id} / {issue_type.value} / {label}")
    ...
    precision_candidates = [row for row in comparison_results if row[5] >= 0.80]
    best_precision_safe = (
