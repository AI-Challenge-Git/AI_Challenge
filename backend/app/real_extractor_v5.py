...
- 스키마 검증(AI-05: NEEDS_CONFIRMATION은 value=null, evidence는 선택)을
  코드에서 우회하지 않고, 위반 시 안전하게 FAILED로 처리하도록 변경 (AI-07)
- FE-07: 날짜 없는 시각을 LLM이 임의의 날짜와 결합해 CONFIRMED로 만드는 경우 차단
- extract_safe()가 (ExtractionResult | None, 실패사유) 형태로 반환
"""
...
def _extract_json_text(raw_content: str) -> str:
    ...
    3. 위 두 경우가 아니면, 첫 '{'부터 중괄호 깊이를 세어 정확히 대응되는
       ...
       이 방식은 설명 문장이 JSON 앞뒤에 붙어 있어도 안전하게 객체
       경계를 찾는다. 문자열 리터럴 밖의 중괄호만 깊이 계산에 사용하므로,
       모델이 evidence_quote 등에 중괄호 문자를 넣어도 오작동하지 않는다.
...
예시:
"10시 30분쯤 주문했다" → attempted_at = NEEDS_CONFIRMATION (시각 단서는 있지만 완전한 날짜 정보가 없음)
...
예시:
"MTS가 이상해요." → {{"value": null, "status": "UNKNOWN", "evidence_quote": null}}
"로그인하려는데 비밀번호가 틀렸다고 나와요." → {{"value": null, "status": "UNKNOWN", "evidence_quote": null}}
"매도 주문을 넣었는데 계속 로딩됩니다." → {{"value": "CUSTOMER_REPORTED_SUBMITTED", "status": "CONFIRMED_FROM_TEXT", "evidence_quote": "매도 주문을 넣었는데"}}
...
reported_occurred_at:
{{"value": "2026-08-15T09:03:00+09:00", "status": "CONFIRMED_FROM_TEXT", "evidence_quote": "2026년 8월 15일 오전 9시 3분에"}}
attempted_at:
{{"value": "2026-08-15T09:03:00+09:00", "status": "CONFIRMED_FROM_TEXT", "evidence_quote": "2026년 8월 15일 오전 9시 3분에"}}
...
class ExtractOutcome(NamedTuple):
    ...
    first_pass_valid: 1차 응답이 재요청 없이 바로 스키마를 통과했는지 여부
    ...
    first_failure_reason / first_failure_detail: 1차 시도가 실패했을 경우
        ...
        2차(retry)까지 실패한 경우 디버깅을 위해 1차 실패 원인도 함께 남긴다.
    """
...
class RealDualExtractor:
    ...
    def _coerce_types(self, field_dict: dict, value_type: type | None) -> dict:
        """
        ...
        규칙과 다르게 쓴 순수 형식 오류다. 새로운 값을 추론해서 채워 넣는 게
        아니라 이미 내린 결론의 표기만 정규화하는 것이므로, hallucination을
        조용히 고치지 않는다는 원칙과 충돌하지 않는다.
        """
    ...
    def _call_llm(self, messages: list[dict]) -> tuple[str | None, ExtractFailureReason | None, str | None]:
        ...
        if content is None or not content.strip():
            # content가 비어있는 경우: max_tokens 초과, content filter, 응답
            # 구조 이상 등 다양한 원인이 있을 수 있다. finish_reason을 함께
            # 남겨서 원인 파악에 참고할 수 있게 한다.
            # nano 모델에서 이 에러가 나면, reasoning trace가 여전히 켜져서
            # max_tokens를 다 소모했을 가능성이 높다 (finish_reason="length" 확인).
    ...
    def _validate_order_type_semantic(self, result: ExtractionResult) -> None:
        """order_type의 value와 명시적 원문 근거가 일치하는지 검증한다."""
    ...
    def _apply_safe_order_type_fallback(
        ...
        """
        원문에 지정가/시장가가 없는데 모델이 주문 방식을 확정한 경우에만
        order_type을 UNKNOWN/null/null로 안전하게 낮춘다.
        """
        order_type = result.consultation.order_type
        ...
        # 원문에 실제 주문 방식 표현이 있으면 잘못된 분류일 수 있으므로
        # 자동 보정하지 않고 기존 retry 경로로 보낸다.
    ...
    def _run_post_validation(
        ...
        """Pydantic 이후 evidence grounding과 semantic validation을 수행한다."""
    ...
    def extract_safe(self, masked_text: str) -> ExtractOutcome:
        ...
        raw hallucination/스키마 위반은 코드가 고쳐주지 않고 정직하게
        INVALID_SCHEMA로 실패 처리한다.

        1차 응답이 스키마 검증에 실패하면, 위반 내용을 알려주는 correction
        메시지와 함께 딱 1회만 재요청한다. 그래도 실패하면 최종 실패로 끝낸다.
        무한 재시도나 코드에서 조용히 값을 고치는 것은 하지 않는다.
        """
        if not masked_text:
        ...
        result, failure_reason, detail = self._parse_and_validate(masked_text, raw_content)
        if result is not None:
            return ExtractOutcome(result, None, None, attempt_count=1, first_pass_valid=True)

        # 1차 실패 원인은 2차까지 실패했을 때 디버깅용으로 함께 반환한다.
        ...
        correction_messages = base_messages + [
            {"role": "assistant", "content": raw_content},
            {
                ...
                "content": (
                    ...
                    "status=UNKNOWN입니다. 모든 evidence_quote는 원문을 요약하거나 "
        ...
        result_2, failure_reason_2, detail_2 = self._parse_and_validate(masked_text, raw_content_2)
...
if __name__ == "__main__":
    ...
    test_text = "10시 30분쯤 [STOCK] 10주 매도했는데 주문 누르고 계속 로딩돼요."
    print("입력:", test_text)
    ...
    if outcome.result is not None:
        print(f"성공 (attempt_count={outcome.attempt_count}, first_pass_valid={outcome.first_pass_valid}):")
